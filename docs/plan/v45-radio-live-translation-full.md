# v45 Plan：实时翻译管线 + MixController + 双模式混音
...
- 两种模式：交替（原声降音量 + 译文朗读）/ 仅译文（原声静音 + 译文朗读）
...
RadioAudioStream（ExoPlayer 正在播放的电台）
  ...
  → TtsService.synthesize(translatedText, priority=BACKGROUND)
  → MixController（混音输出）
...
| TTS 合成 | 1-2 秒 | 单段文本 |
| 端到端 | 9-15 秒 | 用户听到译文时，原声已过去约 10-15 秒 |
...
- `MixController`：控制原声音量 + 播放 TTS 音频，两种模式的状态机
...
class MixController(
    ...
    enum class Mode { INTERLEAVED, TARGET_ONLY }
...
- 被 skip 的 segment 显示文字但标记为灰色 + "未朗读"标签
...
- 真机：开启 live interleaved + target_only 各 2 分钟，观察音量切换、字幕追加、延迟指示器
...
- Tests：
  - `MixController` 状态机单测（两种模式 × 正常/来不及/连续 skip）
  - `LiveTranslationPipeline` 管线编排单测（mock ASR/Translation/TTS，验证串联顺序与超时处理）
  - `AudioSplitter` 切段单测
...
4) TDD Green：实现 `MixController`。
5) TDD Red：`LiveTranslationPipeline` 编排单测 — mock 全部下游，验证 PCM 段 → ASR → Translation → TTS → MixController 的调用顺序与数据传递。
6) TDD Red：`AudioSplitter` 单测 — 输入连续 PCM 流，验证每 5 秒产出一段。
7) TDD Green：实现 `AudioSplitter` + `LiveTranslationPipeline`。
8) TDD Red：CLI `radio live` argv 门禁 + `LiveSessionAlreadyActive` + `RecordingConflict` 测试。
9) TDD Green：实现 CLI + 并发/互斥校验。
10) TDD Green：扩展 `SubtitleScreen` streaming 模式 + 延迟指示器。
11) Verify：UT 全绿；真机冒烟（两种模式各 2 分钟）。
...
- API 费用：实时翻译每分钟消耗 ASR + LLM + TTS 三个 API 调用，费用远高于离线模式。建议 UI 上显示"预估费用"或至少在启动时提示。
