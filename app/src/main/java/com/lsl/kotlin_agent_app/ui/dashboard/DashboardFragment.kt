            ...
            FilesEntryAdapter(
                onClick = { entry ->
                    ...
                    if (entry.type == AgentsDirEntryType.Dir) {
                        ...
                        if (DashboardIrcRules.isIrcDirAtWorkspaceRoot(cwd = cwd, entry = entry)) {
                            startActivity(IrcActivity.intentOf(requireContext()))
                            return@FilesEntryAdapter
                        }
                        ...
                        if (DashboardQqMailRules.isQqMailDirAtWorkspaceRoot(cwd = cwd, entry = entry)) {
                            startActivity(QqMailActivity.intentOf(requireContext()))
                            return@FilesEntryAdapter
                        }
                ...
                onLongClick = { entry ->
                    ...
                    if (DashboardIrcRules.isIrcDirAtWorkspaceRoot(cwd = normalizedCwd, entry = entry)) {
                        val actions = DashboardIrcRules.ircDirLongClickActions()
                        MaterialAlertDialogBuilder(requireContext())
                            .setTitle(entry.name)
                            .setItems(actions) { _, which ->
                                val action = actions.getOrNull(which) ?: return@setItems
                                when (action) {
                                    "进入目录" -> filesViewModel.goInto(entry)
                                    "打开 IRC" -> startActivity(IrcActivity.intentOf(requireContext()))
                                    "剪切" -> filesViewModel.cutEntry(entry)
                                    "复制" -> filesViewModel.copyEntry(entry)
                                    "重命名" -> {
                                        val input =
                    ...
                    if (DashboardSshRules.isSshDirAtWorkspaceRoot(cwd = normalizedCwd, entry = entry)) {
                        val actions = DashboardSshRules.sshDirLongClickActions()
                        MaterialAlertDialogBuilder(requireContext())
                            .setTitle(entry.name)
                            .setItems(actions) { _, which ->
                                val action = actions.getOrNull(which) ?: return@setItems
                    ...
                    if (DashboardQqMailRules.isQqMailDirAtWorkspaceRoot(cwd = normalizedCwd, entry = entry)) {
                        val actions = DashboardQqMailRules.qqmailDirLongClickActions()
                        MaterialAlertDialogBuilder(requireContext())
                            .setTitle(entry.name)
                            .setItems(actions) { _, which ->
                                val action = actions.getOrNull(which) ?: return@setItems
                                when (action) {
                                    "进入目录" -> filesViewModel.goInto(entry)
                                    "打开邮箱" -> startActivity(QqMailActivity.intentOf(requireContext()))
                                    "剪切" -> filesViewModel.cutEntry(entry)
                                    "复制" -> filesViewModel.copyEntry(entry)
                                    "重命名" -> {
                                        val input =
                    ...
                    if (DashboardInstantTranslationRules.isInstantTranslationDirAtWorkspaceRoot(cwd = normalizedCwd, entry = entry)) {
                        val actions = DashboardInstantTranslationRules.instantTranslationDirLongClickActions()
                        MaterialAlertDialogBuilder(requireContext())
                            .setTitle(entry.name)
                            .setItems(actions) { _, which ->
